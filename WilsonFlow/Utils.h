...
double maxNorm(const LatticeGaugeField& U) {
  ...
  accelerator_for(ss, U_norm2.Grid()->oSites(), vComplex::Nsimd(), {
    for(int mu=0; mu<Nd; mu++) {
      U_norm2_v[ss](mu)()() = 0.;
      for(int c1=0;c1<Nc;c1++)
        for(int c2=0;c2<Nc;c2++)
            // U_norm2[ss](mu)()() += toReal(U[ss](mu)()(c1, c2) * U[ss](mu)()(c1, c2));
            U_norm2_v[ss](mu)()() += toReal(conjugate(U_v[ss](mu)()(c1, c2)) * U_v[ss](mu)()(c1, c2));
    }
  });
  ...
  for(int ss=0;ss<U_norm2.Grid()->oSites();ss++)
  {
    for(int mu=0; mu<Nd; mu++) {
      // scalar_type *sobj = (scalar_type *)& U_norm2[ss](mu)()();
      scalar_type *sobj = (scalar_type *)& U_norm2_v[ss](mu)()();
      for(int idx=0; idx<U_norm2.Grid()->Nsimd(); ++idx)
        if( *(sobj + idx) > max_val)
            max_val = *(sobj + idx);
    }
  }
  ...
  MPI_Allreduce(MPI_IN_PLACE, &max_val, 1, MPI_DOUBLE, MPI_MAX, U_norm2.Grid()->communicator);
  #endif
